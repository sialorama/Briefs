...
class Connexion:
    ...
    @classmethod
    def get_idclient(cls, client):
        cls.ouvrir_connexion()
        cls.cursor.execute(f"Select id_client from clients WHERE clients.nom_client = '{client}'")
        client = cls.cursor.fetchall()
        cls.fermer_connexion
        return client

    @classmethod
    def get_idproduit(cls, produit):
        cls.ouvrir_connexion()
        cls.cursor.execute(f"Select id_produit from produits WHERE nom_produit = '{produit}'")
        produit = cls.cursor.fetchall()
        cls.fermer_connexion
        return produit
    ...
    def saisie_facture(cls, date, client, facture, produit, quantite):
        cls.ouvrir_connexion()
        client = cls.get_idclient
        produit = cls.get_idproduit
        cls.cursor.execute(f"INSERT INTO factures ({facture}, {date}, {client})")
        cls.cursor.execute(f"INSERT INTO facture_produit ({facture}, {produit}, {quantite});")
        cls.cursor.commit
        cls.fermer_connexion
        return